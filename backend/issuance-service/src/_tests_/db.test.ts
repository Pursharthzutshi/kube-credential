...
import { connectDB, getIssuedCollection, closeDB } from '../db';
...
describe('Database Module', () => {
  ...
  describe('closeDB', () => {
    it('should be a mockable function', () => {
      expect(mockCloseDB).toBeDefined();
      expect(typeof mockCloseDB).toBe('function');
    });

    it('should handle async operations', async () => {
      mockCloseDB.mockResolvedValue(undefined);
      await expect(mockCloseDB()).resolves.toBeUndefined();
    });
  });
});
